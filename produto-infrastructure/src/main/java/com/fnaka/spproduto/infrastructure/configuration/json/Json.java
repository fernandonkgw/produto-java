package com.fnaka.spproduto.infrastructure.configuration.json;
...
public enum Json {
    ...
    public static <T> T readValue(final String json, final Class<T> type) {
        return invoke(() -> INSTANCE.mapper.readValue(json, type));
    }
    ...
    private final ObjectMapper mapper = new Jackson2ObjectMapperBuilder()
            .dateFormat(new StdDateFormat()) // "yyyy-MM-dd'T'HH:mm:ss.SSS"
            .featuresToDisable(
            ...
            .modules(
                    new JavaTimeModule(), // serializa pacotes do javatime localdate, localdatetime, instant
                    new Jdk8Module(), // optinal
                    afterburnerModule() // otimiza a serialização
            )
